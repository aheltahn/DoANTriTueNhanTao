...
const VietnamMapWithProvinces = ({ onGbfsResult, onWcoResult }) => {
  ...
  const handleGetCurrentLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        const calibratedCoords = calibrateCoordinatesForVietnam(
          latitude,
          longitude
        );
      ...
      (error) => {
        ...
        alert(
          "Không thể lấy vị trí chính xác. Hãy thử lại trên điện thoại hoặc HTTPS."
        );
      },
      {
  ...
  const handleRemoveCurrentLocation = () => {
    ...
    setSelectedProvinces((prev) =>
      prev.filter((p) => p.id !== currentLocation.id)
    );
  ...
  const handleCalculateRoute = async () => {
    ...
    try {
      ...
      if (startingPoint.id === currentLocation?.id) {
        ...
        const otherProvinces = selectedProvinces.filter(
          (p) => p.id !== startingPoint.id
        );
        cities = [
          {
            name: startingPoint.name,
  ...
  const handleResetAll = () => {
    if (window.confirm(" Bạn có chắc muốn xóa tất cả và bắt đầu lại không?")) {
    ...
    <div style={{ height: "100vh" }}>
      <div className="max-w-[1400px] mx-auto rounded-xl shadow-2xl h-full flex flex-col overflow-hidden">
        ...
        <div className="flex-shrink-0 p-5 ">
          <h1 className="text-5xl font-extrabold mb-1 text-white">
            {" "}
            BẢN ĐỒ THÀNH PHỐ VIỆT NAM
          </h1>
          <p className="text-sm text-blue-100">
            Khám phá các điểm đến và lập kế hoạch hành trình của bạn
          </p>
        </div>
                  ...
                  <div className="flex justify-between items-center">
                    <div className="text-sm font-medium text-white">
                      ...
                      {startingPoint.id === currentLocation?.id && (
                        <span className="text-[#9dfd7b] ml-2">
                          (Vị trí hiện tại)
                        </span>
                      )}
                    </div>
  ...
  `}
              ...
              {!startingPoint && (
                <div className="text-xs text-red-500 text-center">
                  Vui lòng chọn điểm xuất phát
                </div>
              )}
              {startingPoint && selectedProvinces.length < 2 && (
                <div className="text-xs text-red-500 text-center">
                  Chọn ít nhất 1 tỉnh khác để tính đường đi
                </div>
              )}
              ...
              {/* {tspResult && (
                <div className="mt-4 p-3 bg-green-50 border border-green-200 rounded text-sm">
                  ...
                  <div><strong> Lộ trình:</strong> {tspResult.best_solution.join(' → ')}</div>
                  <div><strong> Khoảng cách:</strong> {tspResult.best_distance} km</div>
                  <div><strong> Thời gian:</strong> {tspResult.execution_time} giây</div>
